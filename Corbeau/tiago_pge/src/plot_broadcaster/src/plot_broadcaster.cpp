...
/*---------- LISTE DE POINTS D APPROCHE BASE_FOOTPRINT -----------*/
  ...
  std::vector <std::vector<double> > cercle = gen_poi.generate_circle(__NB_POINTS_BASE_FOOTPRINT__, 1.70, 0, 0, 0);
  ...
  /*--------Publication des frames correspondants aux plots -----------*/
    ...
    while(node.ok()){
        ...
        for(iPoint = 0 ; iPoint < __NB_POINTS_BASE_FOOTPRINT__ ; iPoint++){
            ...
            ensembleFramePlot[iPoint].setRotation(tf::Quaternion(0,0,atan2(cercle[1][iPoint],cercle[0][iPoint])+ (M_PI/2))); //Fait déjà la conversion rawpitchyall
